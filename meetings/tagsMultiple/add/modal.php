...
if($validateFlag == 200){
?>
	<h1>Tilføj mærke på markerede identiteter</h1>
	<form action="/identities/tagsMultiple/add/modalSubmit.php" enctype="application/x-www-form-urlencoded" method="post" onsubmit="submitForm(this);" target="<?php echo md5($_SERVER['SCRIPT_FILENAME']) . purify($modalId); ?>">
		...
		<div>
			Tilføj nedenstående mærke på <?php echo substr_count(purify($identities_id),',') +1; ?> <?php if(substr_count(purify($identities_id),',') == 0){echo 'entitet';}else{echo 'identiteter';} ?>.<br>
			<br>
