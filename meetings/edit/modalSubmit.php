...
if($validateFlag == 200){
	...
	$stmt->prepare("
		...
		WHERE
			`c0`.`identities`.`id` = ?
	");
	$stmt->bind_param('ssssssssssssssi', $type, $name, $name2, $cvrNumber, $cprNumber, $address, $address2, $zipCode, $city, $country, $phone, $phone2, $email, $email2, $identities_id);
	$stmt->execute();
	...
	<script>
		...
		parent.toastr('success', 'Editer identitet', 'Identiteten blev editeret.', 0, true, '');
	</script>
	<?php
