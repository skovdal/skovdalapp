...
if(isset($_POST['encryptionSystemNotifications']) === false){
	$validateFlag = 400;
}
else{
	$encryptionSystemNotifications = $_POST['encryptionSystemNotifications'];
}

if(isset($_POST['logSystemNotifications']) === false){
	$validateFlag = 400;
}
else{
	$logSystemNotifications = $_POST['logSystemNotifications'];
}
...
if($validateFlag == 200){
	...
	$stmt->prepare("
		...
		WHERE
			`s0`.`systemConfigurations`.`systemConfiguration` = 'encryptionSystemNotifications'
			OR
			`s0`.`systemConfigurations`.`systemConfiguration` = 'logSystemNotifications'
	");
	...
	(
		?,
		'encryptionSystemNotifications'
	)
	");
	$stmt->bind_param('i', $encryptionSystemNotifications);
	$stmt->execute();
	setTableVersion('systemConfigurations');
	...
	(
		?,
		'logSystemNotifications'
	)
	");
	$stmt->bind_param('i', $logSystemNotifications);
	$stmt->execute();
	setTableVersion('systemConfigurations');
	?>
	<script>
