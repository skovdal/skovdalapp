...
if(isset($_POST['systemNotifications_id']) === false){
	...
	foreach($systemNotifications_id_array as $systemNotifications_id){
		if(decodeId($systemNotifications_id) == -1){
			...
			if(isset($systemNotifications_ids) === false){
				$systemNotifications_ids = $systemNotifications_id;
			}
			else{
				$systemNotifications_ids = $systemNotifications_ids . ',' . $systemNotifications_id;
			}
		}
	}
	$systemNotifications_id = $systemNotifications_ids;
	$systemNotifications_id_array = explode(',', $systemNotifications_id);
}
...
if($validateFlag == 200){
	...
	foreach($systemNotifications_id_array as $systemNotifications_id){
		...
		");
		$stmt->bind_param('ii', $tags_id, $systemNotifications_id);
		$stmt->execute();
	...
	<script>
		...
		parent.toastr('success', 'Tilføj mærke på markerede systemnotifikationer', 'Mærket blev tilføjet på de markerede systemnotifikationer.', 0, true, '');
	</script>
	<?php
