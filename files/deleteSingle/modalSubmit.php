...
if($validateFlag == 200){
	...
	if($filesMetaDataWithSharedFilesContent_count == 1){
		if(isset($con) === false){$con = dbConnection();}
		$stmt = $con->stmt_init();
		$stmt->prepare("
			UPDATE
	...
	<script>
		...
		parent.toastr('success', 'Slet fil', 'Filen blev slettet.', 0, true, '');
	</script>
	<?php
...
?>
	<script>
		...
		parent.document.querySelectorAll('#modal-<?php echo purify($modalId); ?> input.delete[type="submit"]')[0].value = 'Slet fil';
