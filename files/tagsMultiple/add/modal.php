...
if($validateFlag == 200){
?>
	<h1>Tilføj mærke på markerede filer</h1>
	<form action="/files/tagsMultiple/add/modalSubmit.php" enctype="application/x-www-form-urlencoded" method="post" onsubmit="submitForm(this);" target="<?php echo md5($_SERVER['SCRIPT_FILENAME']) . purify($modalId); ?>">
		...
		<input name="file_id" type="hidden" value="<?php echo purify($file_id); ?>">
		<div>
			Tilføj nedenstående mærke på <?php echo substr_count(purify($file_id),',') +1; ?> <?php if(substr_count(purify($file_id),',') == 0){echo 'fil';}else{echo 'filer';} ?>.<br>
			<br>
