...
?>
	...
	<form action="/files/add/modalSubmit.php?modalId=<?php echo urlencode(purify($modalId)); ?>" enctype="multipart/form-data" method="post" target="<?php echo md5($_SERVER['SCRIPT_FILENAME']) . purify($modalId); ?>">
		...
		<div class="contentTab">
			...
			<div class="dragDropFileContainer" ondragover="this.style.border='1px solid var(--color-focus)';" ondragleave="this.style.border='1px solid rgba(225,230,235,1)';" ondrop="function(event){event.preventDefault();}">
				<div class="noContent">
					<input type="hidden" name="MAX_FILE_SIZE" value="3000000000">
					<input type="file" name="files[]" id="fileFiles" multiple required>
					<br><br><br><br><br><br><br><br><br>Træk en eller flere filer hertil eller <label for="fileFiles">klik for at vælge</label>
				</div>
			</div>
		</div>
		
		<div class="buttons">
			<input class="close" onclick="document.querySelectorAll('#modal-<?php echo purify($modalId); ?> div.close')[0].click();" type="button" value="Luk"><input type="submit" value="Gem filer">
		</div>
	</form>
	<form action="/files/add/modalSubmit.php" enctype="application/x-www-form-urlencoded" method="post" onsubmit="submitForm(this);" target="<?php echo md5($_SERVER['SCRIPT_FILENAME']) . purify($modalId); ?>">
		<div class="tagTab">
			...
			<input class="addTag" onclick="modal(0, 'basic', '/files/add/tags/add/modal.php', 'POST', '&refererModalId=<?php echo purify($modalId); ?>', true, 1);" type="button" value="Tilføj mærke" autofocus>
		</div>
