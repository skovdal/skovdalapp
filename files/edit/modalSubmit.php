...
if($validateFlag == 200){
	...
	$stmt->prepare("
		...
		SET
			...
			`c0`.`filesMetaData`.`name2` = ?,
			`c0`.`filesMetaData`.`cvrNumber` = ?,
			`c0`.`filesMetaData`.`cprNumber` = ?,
			`c0`.`filesMetaData`.`address` = ?,
			`c0`.`filesMetaData`.`address2` = ?,
			`c0`.`filesMetaData`.`zipCode` = ?,
			`c0`.`filesMetaData`.`city` = ?,
			`c0`.`filesMetaData`.`country` = ?,
			`c0`.`filesMetaData`.`phone` = ?,
			`c0`.`filesMetaData`.`phone2` = ?,
			`c0`.`filesMetaData`.`email` = ?,
			`c0`.`filesMetaData`.`email2` = ?
		WHERE
			`c0`.`filesMetaData`.`id` = ?
	");
	$stmt->bind_param('ssssssssssssssi', $type, $name, $name2, $cvrNumber, $cprNumber, $address, $address2, $zipCode, $city, $country, $phone, $phone2, $email, $email2, $files_id);
	$stmt->execute();
	...
	foreach($_POST as $postName => $postValue){
		...
		$stmt = $con->stmt_init();
			...
			");
			$stmt->bind_param('ii', $tags_id, $files_id);
			$stmt->execute();
			setTableVersion('tagsReferences');
		}
	}
	...
	<script>
		...
		parent.toastr('success', 'Editer fil', 'Filen blev editeret.', 0, true, '');
	</script>
	<?php
