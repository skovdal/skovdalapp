...
if(isset($_POST['modalId']) === false){
	...
	$validateFlagOpenUrl = 'https://errors.complian.app.complian.dev?1234-ABCD-5678-EFGH';
}
else{
...
if(isset($_POST['identities_id']) === false){
	...
	foreach($identities_id_array as $identities_id){
		if(decodeId($identities_id) == -1){
			...
			if(isset($identities_ids) === false){
				$identities_ids = $identities_id;
			}
			else{
				$identities_ids = $identities_ids . ',' . $identities_id;
			}
		}
	}
	$identities_id = $identities_ids;
	$identities_id_array = explode(',', $identities_id);
}
...
if($validateFlag == 200){
	$totalCounter = 0;
	$deletedCounter = 0;
	$identities_ids = explode(',', $identities_id);
	foreach($identities_ids as $identities_id){
		...
		if(mysqli_num_rows($result) > 0){
			...
			$validateFlagTitle = 'Slet markerede identiteter';
			$validateFlagMsg = 'Det er desværre ikke muligt at slette en eller flere af de markerede identiteter, da de har tilknyttede systembrugere.<br><br>Fjern venligst tilknytningerne for at slette identiteterne.';
			$validateFlagToastrId = 0;
			$validateFlagRegister = true;
			$validateFlagOpenUrl = '';
		}
