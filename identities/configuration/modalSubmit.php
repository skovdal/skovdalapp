...
if(isset($_POST['encryptionIdidentities']) === false){
	$validateFlag = 400;
}
else{
	$encryptionIdidentities = $_POST['encryptionIdidentities'];
}

if(isset($_POST['logIdidentities']) === false){
	$validateFlag = 400;
}
else{
	$logIdidentities = $_POST['logIdidentities'];
}

if(isset($_POST['deleteConfirmationIdidentities']) === false){
	$validateFlag = 400;
}
else{
	$deleteConfirmationIdidentities = $_POST['deleteConfirmationIdidentities'];
}
...
if($validateFlag == 200){
	...
	$stmt->prepare("
		...
		WHERE
			`s0`.`systemConfigurations`.`systemConfiguration` = 'encryptionIdidentities'
			OR
			`s0`.`systemConfigurations`.`systemConfiguration` = 'logIdidentities'
			OR
			`s0`.`systemConfigurations`.`systemConfiguration` = 'deleteConfirmationIdidentities'
	");
	...
	(
		?,
		'encryptionIdidentities'
	)
	");
	$stmt->bind_param('i', $encryptionIdidentities);
	$stmt->execute();
	setTableVersion('systemConfigurations');
	...
	");
	$stmt->bind_param('i', $logIdidentities);
	$stmt->execute();
	setTableVersion('systemConfigurations');
	...
	(
		?,
		'deleteConfirmationIdidentities'
	)
	");
	$stmt->bind_param('i', $deleteConfirmationIdidentities);
	$stmt->execute();
	setTableVersion('systemConfigurations');
	?>
	<script>
		...
		parent.toastr('success', 'Sikkerhedsindstillinger for identiteter', 'Sikkerhedsindstillingerne blev gemt.', 0, true, '');
	</script>
	<?php
