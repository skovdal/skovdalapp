...
if($validateFlag == 200){
?>
	<h1>Eksporter markerede identiteter</h1>
	<ul>
	...
	<form action="/identities/exportMultiple/modalSubmit.php" enctype="application/x-www-form-urlencoded" method="post" onsubmit="submitForm(this);" target="<?php echo md5($_SERVER['SCRIPT_FILENAME']) . purify($modalId); ?>">
		...
		<div>
			Eksporter <?php echo substr_count(purify($identities_id),',') +1; ?> <?php if(substr_count(purify($identities_id),',') == 0){echo 'entitet';}else{echo 'identiteter';} ?>.<br>
			<br>
			<input id="inputName" name="name" pattern=".{3,}" placeholder="identiteter" type="text" value="identiteter" required autofocus><label for="inputName">Filnavn</label><br>
