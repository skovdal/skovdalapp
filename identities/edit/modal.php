...
?>
	...
	<form action="/identities/edit/modalSubmit.php" enctype="application/x-www-form-urlencoded" method="post" onsubmit="submitForm(this);" target="<?php echo md5($_SERVER['SCRIPT_FILENAME']) . purify($modalId); ?>">
		...
		<div>
			...
			<select class="photo" id="inputType" name="type" autofocus required>
				...
				<option value="Offentlig myndighed" <?php if($identities_type == 'Offentlig myndighed'){echo 'selected';} ?>>Offentlig myndighed</option>
			...
			<input class="photo" id="inputName" name="name" pattern=".{3,}" placeholder="Jens Nielsen" type="text" value="<?php echo purify($identities_name); ?>" required><label for="inputName">Navn</label><br>
			<input class="photo" id="inputName2"  name="name2" pattern=".{3,}" placeholder="Økonomiafdelingen" type="text" value="<?php echo purify($identities_name2); ?>"><label for="inputName2">Supplerende adresseoplysninger</label><br>
		...
		<div class="buttons">
			<input class="delete" onclick="modal(0, 'basic', '/identities/edit/delete/modal.php', 'POST', '&refererModalId=<?php echo purify($modalId); ?>&identities_id=<?php echo encodeId(purify($identities_id)); ?>', true, 1);" type="button" value="Slet identitet"><input class="close" onclick="document.querySelectorAll('#modal-<?php echo purify($modalId); ?> div.close')[0].click();" type="button" value="Luk"><input type="submit" value="Gem identitet">
		</div>
	</form>
	<?php
