...
if($validateFlag == 200){
?>
	<h1>Eksporter markerede behandlingsaktiviteter</h1>
	<ul>
	...
	<form action="/processingActivities/exportMultiple/modalSubmit.php" enctype="application/x-www-form-urlencoded" method="post" onsubmit="submitForm(this);" target="<?php echo md5($_SERVER['SCRIPT_FILENAME']) . purify($modalId); ?>">
		...
		<div>
			Eksporter <?php echo substr_count(purify($processingActivities_id),',') +1; ?> <?php if(substr_count(purify($processingActivities_id),',') == 0){echo 'behandlingsaktivitet';}else{echo 'behandlingsaktiviteter';} ?>.<br>
			<br>
			<input id="inputName" name="name" pattern=".{3,}" placeholder="behandlingsaktiviteter" type="text" value="behandlingsaktiviteter" required autofocus><label for="inputName">Filnavn</label><br>
