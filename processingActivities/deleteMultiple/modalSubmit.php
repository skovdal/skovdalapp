...
if($validateFlag == 200){
	...
	$stmt->prepare("
		...
		WHERE
			`c0`.`processingActivities`.`id` IN (" . $processingActivities_id . ")
	");
	$stmt->execute();
	...
	$stmt->prepare("
		...
		WHERE
			`s0`.`tagsReferences`.`processingActivities_id` IN (" . $processingActivities_id . ")
	");
	$stmt->execute();
...
?>
	<script>
		...
		parent.document.querySelectorAll('#modal-<?php echo purify($modalId); ?> input.delete[type="submit"]')[0].value = 'Slet markerede behandlingsaktiviteter';
