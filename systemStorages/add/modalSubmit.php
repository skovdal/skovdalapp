...
if(isset($_POST['storageSize']) === false){
	$validateFlag = 400;
}
else{
...
if(isset($_POST['ftpRemotePath']) === false){
	$validateFlag = 400;
}
else{
	$ftpRemotePath = $_POST['ftpRemotePath'];
}

if(isset($_POST['mySqlSocket']) === false){
	$validateFlag = 400;
}
else{
	$mySqlSocket = $_POST['mySqlSocket'];
}

if(isset($_POST['ftpPassiveMode']) === false){
	$ftpPassiveMode = 0;
}
else{
	$ftpPassiveMode = 1;
...
if($validateFlag == 200){
	...
	$stmt->prepare("
		...
		(
			...
			`c0`.`systemStorages`.`type`,
			`c0`.`systemStorages`.`storageSize`,
			`c0`.`systemStorages`.`ftp_connect_host`,
			`c0`.`systemStorages`.`ftp_connect_port`,
			...
			`c0`.`systemStorages`.`mysql_host`,
			`c0`.`systemStorages`.`mysql_username`,
			`c0`.`systemStorages`.`mysql_password`,
			`c0`.`systemStorages`.`mysql_dbname`,
			`c0`.`systemStorages`.`mysql_port`,
			`c0`.`systemStorages`.`mysql_socket`
		)
		VALUES
		(
			?,
