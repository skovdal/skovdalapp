...
if($validateFlag == 200){
?>
	<h1>Eksporter markerede systemhændelser</h1>
	<ul>
	...
	<form action="/systemEvents/exportMultiple/modalSubmit.php" enctype="application/x-www-form-urlencoded" method="post" onsubmit="submitForm(this);" target="<?php echo md5($_SERVER['SCRIPT_FILENAME']) . purify($modalId); ?>">
		...
		<input name="systemEvents_id" type="hidden" value="<?php echo purify($systemEvents_id); ?>">
		<div>
			Eksporter <?php echo substr_count(purify($systemEvents_id),',') +1; ?> <?php if(substr_count(purify($systemEvents_id),',') == 0){echo 'systemhændelse';}else{echo 'systemhændelser';} ?>.<br>
			<br>
			<input id="inputName" name="name" pattern=".{3,}" placeholder="systemhændelser" type="text" value="systemhændelser" required autofocus><label for="inputName">Filnavn</label><br>
