...
if(isset($_POST['encryptionSystemEvents']) === false){
	$validateFlag = 400;
}
else{
	$encryptionSystemEvents = $_POST['encryptionSystemEvents'];
}

if(isset($_POST['logSystemEvents']) === false){
	$validateFlag = 400;
}
else{
	$logSystemEvents = $_POST['logSystemEvents'];
}
...
if($validateFlag == 200){
	...
	$stmt->prepare("
		...
		WHERE
			`s0`.`systemConfigurations`.`systemConfiguration` = 'encryptionSystemEvents'
			OR
			`s0`.`systemConfigurations`.`systemConfiguration` = 'logSystemEvents'
	");
	...
	(
		?,
		'encryptionSystemEvents'
	)
	");
	$stmt->bind_param('i', $encryptionSystemEvents);
	$stmt->execute();
	setTableVersion('systemConfigurations');
	...
	");
	$stmt->bind_param('i', $logSystemEvents);
	$stmt->execute();
	setTableVersion('systemConfigurations');
	?>
	<script>
		...
		parent.toastr('success', 'Sikkerhedsindstillinger for systemhændelser', 'Sikkerhedsindstillingerne blev gemt.', 0, true, '');
	</script>
	<?php
