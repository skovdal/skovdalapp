...
?>
	...
	<form action="/systemEvents/edit/modalSubmit.php" enctype="application/x-www-form-urlencoded" method="post" onsubmit="submitForm(this);" target="<?php echo md5($_SERVER['SCRIPT_FILENAME']) . purify($modalId); ?>">
		...
		<input name="systemEvents_id" type="hidden" value="<?php echo encodeId(purify($systemEvents_id)); ?>">
		<div>
		...
		$stmt = $con->stmt_init();
			...
			<input class="addTag" onclick="modal(0, 'basic', '/systemEvents/edit/tags/add/modal.php', 'POST', '&refererModalId=<?php echo purify($modalId); ?>&systemEvents_id=<?php echo encodeId(purify($systemEvents_id)); ?>', true, 1);" type="button" value="Tilføj mærke" autofocus>
		...
		<div class="buttons">
			<input class="close" onclick="document.querySelectorAll('#modal-<?php echo purify($modalId); ?> div.close')[0].click();" type="button" value="Luk"><input type="submit" value="Gem systemhændelse">
		</div>
	</form>
	<?php
