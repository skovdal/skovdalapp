...
if(isset($_POST['systemEvents_id']) === false){
	...
	foreach($systemEvents_id_array as $systemEvents_id){
		if(decodeId($systemEvents_id) == -1){
			...
			if(isset($systemEvents_ids) === false){
				$systemEvents_ids = $systemEvents_id;
			}
			else{
				$systemEvents_ids = $systemEvents_ids . ',' . $systemEvents_id;
			}
		}
	}
	$systemEvents_id = $systemEvents_ids;
	$systemEvents_id_array = explode(',', $systemEvents_id);
}
...
if($validateFlag == 200){
	...
	foreach($systemEvents_id_array as $systemEvents_id){
		...
		");
		$stmt->bind_param('ii', $tags_id, $systemEvents_id);
		$stmt->execute();
