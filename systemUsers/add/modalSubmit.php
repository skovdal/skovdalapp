...
if($validateFlag == 200){
	...
	$stmt->prepare("
		...
		(
			`c0`.`systemUsers`.`name`,
			`c0`.`systemUsers`.`securityClearance`,
			`c0`.`systemUsers`.`blockSignIn`,
			`c0`.`systemUsers`.`userMustChangePasswordAtNextSignIn`,
			`c0`.`systemUsers`.`forceUseOfMultifactorAuthentication`,
	...
	");
	$stmt->bind_param('ssiiii', $name, $securityClearance, $blockSignIn, $userMustChangePasswordAtNextSignIn, $forceUseOfMultifactorAuthentication, $identities_id);
	$stmt->execute();
	$systemUsers_id = mysqli_insert_id($con);
	...
	<script>
		...
		parent.toastr('success', 'Ny systembruger', 'Systembrugeren blev tilføjet.', 0, true, '');
	</script>
	<?php
